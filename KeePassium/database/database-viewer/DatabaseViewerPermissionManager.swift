...
typealias DatabaseViewerItemPermissions = Set<DatabaseViewerPermissionManager.PermissionElement>
...
enum DatabaseViewerPermissionManager {
    ...
    private static let possibleInReadOnly = Permissions([.printDatabase])
    ...
    private static func trimPermissions(_ permissions: inout Permissions, for group: Group) {
        if !permissions.contains(.editDatabase) {
            permissions = permissions.intersection(Self.possibleInReadOnly)
        }
        ...
        permissions.remove(.selectItems, if: isGroupEmpty && !group.isSmartGroup)
        permissions.remove(
            .reorderItems,
    ...
    private static func gatherPermissions(
        ...
        guard canEditDatabase else {
            return Self.possibleInReadOnly
        }
